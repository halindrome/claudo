...
rotate_log_if_needed() {
  ...
  if gzip -9 < "$log_file" > "${log_file}.1.gz" 2>/dev/null; then
    : > "$log_file"
  else
    warn "Log rotation skipped: gzip failed (is gzip installed?)"
    rm -f "${log_file}.1.gz" 2>/dev/null || true
  fi
}
...
PYEOF
  ...
  if [[ -f "${MODELS_CACHE}" ]]; then
    # date -r works on both macOS (BSD date) and Linux (GNU coreutils 8.1+)
    cache_time=$(date -r "${MODELS_CACHE}" "+%Y-%m-%d %H:%M" 2>/dev/null || echo "unknown")
  fi
  ...
  while [[ $_i -le $# ]]; do
    case "${!_i}" in
      --profile|-P)
        _i=$((_i + 1))
        if [[ $_i -gt $# ]]; then
          die "--profile requires a name argument"
        fi
        if [[ -z "${!_i}" ]]; then
